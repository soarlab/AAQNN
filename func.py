...
def getModelFromQNN(cf, train_x, train_y, test_x, test_y):
    ...
    checkpoint = ModelCheckpoint(cf.out_wght_path, monitor='val_acc', verbose=1, save_best_only=True, mode='max',
                                 period=1)
    tensorboard = TensorBoard(log_dir='./logs/' + str(cf.tensorboard_name), histogram_freq=0, write_graph=True,
                              write_images=False)
    print('loading data\n')
